    ...
    if (user != "" && pass != "") {
      await axios
        ...
        .catch(function (error) {
          console.log(error);
          Alert.alert("Error", error.toString().replace("AxiosError: ", ""), [
            {
              text: "Ok",
            },
          ]);
...
export default function LoginPage() {
  ...
  return (
    <View className=" h-full bg-slate-200 dark:bg-zinc-950">
      <SafeAreaView className=" h-full bg-slate-200 dark:bg-zinc-950">
        ...
        <View className="relative h-screen">
          ...
          <View className="text-center mx-auto justify-center w-64 h-64">
            <Pressable
              onPress={() => {
                Login(APIUrl, User, Pass);
                console.log("Pressed");
              }}
              className="m-auto bg-zinc-950 shadow-lg dark:bg-slate-200 h-16 w-32 bottom-0 rounded-full active:bg-zinc-600 active:dark:bg-slate-300"
            >
